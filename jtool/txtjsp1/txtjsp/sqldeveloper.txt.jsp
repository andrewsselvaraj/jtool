...
<%
		...
		try
		{
			...
			out.println("<form action=\"querydisp.jsp\">");
			String header="<table border=\"1\"><TR><td>&nbsp</td><td>Column Name</td></tr>";
			while(i<=totcol)
				{

//				header = header+"<td>"+rsm.getColumnName(i)+"</td><td>"+rsm.getColumnDisplaySize(i)+"</td><td>"+rsm.getColumnTypeName(i)+"</td></tr>";
				header = header+"<td><INPUT type='checkbox' name='fieldname' value ="+rsm.getColumnName(i)+"></td><td>"+rsm.getColumnName(i)+"</td></tr>";
				i = i+1;
				}
			header=header + "<tr><td colspaan=2 ><INPUT type='submit' name='getqry'></td></tr></table>";
			header= header+"</form>";
			rsm = null;			
			rs.close();
