...
<%
		...
		try
			...
			con=DriverManager.getConnection("jdbc:odbc:codegen", "codegen","codegen");
			st=con.createStatement();
			String usecase_id = request.getParameter("usecase_id");
			String classname = request.getParameter("classname");

			String 	sql = "insert into CLASSMASTER  values (";
					sql = sql +"'"+usecase_id+"',";
					sql = sql +"CLASS_id_seq.nextval,";
					sql = sql +"'"+classname+"')";					
			out.println("SQL Query :="+sql);
			int i=st.executeUpdate(sql);
			if(i>0)
				{
								out.println(classname+"Inserted Successfully");
				}
